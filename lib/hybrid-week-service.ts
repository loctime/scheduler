// Servicio Híbrido Controlado - Sistema de Semanas
...
export class HybridWeekService {
  ...
  static async getWeekData(baseWeekId: string | null, weekStart: string, ownerId: string): Promise<{
    ...
    try {
      // 1. Primero verificar si existe en schedules
   ...
   * Crea baseWeekId si no existe y estructura completa de versionado
   */
  static async markWeekComplete(
    ...
    try {
      ...
      return await runTransaction(db, async (transaction) => {
        ...
        // 2. Crear documento base de semana si no existe
        ...
        if (!weekDoc.exists()) {
          ...
          transaction.update(weekRef, weekDocument)
        }
  ...
  /**
   * REGLA 5: Editar semana legacy (sin baseWeekId)
   * Usa el sistema schedules existente
   */
  static async editLegacyWeek(
    weekStart: string,
    ownerId: string,
    date: string,
    employeeId: string,
    ...
    // REGLA: Delegar al sistema legacy existente
    // Esta función mantiene la compatibilidad con schedules no versionados
    throw new Error("Usar scheduleApplication.updateAssignment para semanas legacy")
  }
