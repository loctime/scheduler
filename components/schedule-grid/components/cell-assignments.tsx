...
export function CellAssignments({ assignments, getShiftInfo }: CellAssignmentsProps) {
  ...
  if (assignments.length === 0) {
    return <span className="text-center text-base sm:text-lg md:text-xl lg:text-2xl text-muted-foreground font-medium">-</span>
  }
    ...
    <>
      {orderedAssignments.map((assignment, idx) => {
        if (assignment.type === "franco") {
          return (
            <span key={`franco-${idx}`} className="text-center text-sm sm:text-base md:text-lg lg:text-xl font-bold block">
              FRANCO
            </span>
          )
        }
        ...
        if (assignment.type === "medio_franco") {
          ...
          if (hasTime) {
            if (hasShifts) {
              ...
              return (
                <span key={`medio-franco-${idx}`} className="block text-center text-sm sm:text-base md:text-lg lg:text-xl font-bold text-[#22c55e]">
                  (1/2 Franco)
                </span>
              )
            } else {
              ...
              if (isEarly) {
                return (
                  <div key={`medio-franco-${idx}`} className="text-center text-sm sm:text-base md:text-lg lg:text-xl">
                    <span className="block font-semibold">{displayTimeLines[0]}</span>
                    <span className="block text-xs sm:text-sm md:text-base font-bold text-[#22c55e]">(1/2 Franco)</span>
                  </div>
                )
              } else {
                return (
            ...
            return (
              <span key={`medio-franco-${idx}`} className="block text-center text-sm sm:text-base md:text-lg lg:text-xl font-semibold">
                1/2 Franco
              </span>
            )
          }
        ...
        if (!displayTimeLines || displayTimeLines.length === 0 || (displayTimeLines.length === 1 && !displayTimeLines[0])) {
          return (
            <span key={assignment.shiftId} className="text-center text-sm sm:text-base md:text-lg lg:text-xl font-semibold block">
              {shift.name}
            </span>
          )
        }
        ...
        return (
          <div key={assignment.shiftId} className="text-center text-sm sm:text-base md:text-lg lg:text-xl">
