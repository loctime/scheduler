...
export interface PedidosTabsProps {
  ...
  remitosCount: number
  showPedirInsumosShortcut?: boolean
  onOpenPedirInsumos?: () => void
}
...
export function PedidosTabs({
  ...
  selectedPedido,
  remitosCount,
  showPedirInsumosShortcut = false,
  onOpenPedirInsumos
}: PedidosTabsProps) {
    ...
    <div className="flex gap-0.5 sm:gap-1 border-t sm:border-t-0 border-l sm:border-l sm:border-r border-border overflow-x-hidden">
      ...
      {showPedirInsumosShortcut && (
        <Button
          ...
          onClick={onOpenPedirInsumos}
        >
          <span className="hidden sm:inline">Pedir insumos</span>
