...
export interface UserData {
  ...
  ownerId?: string
  locationId?: string
  grupoIds?: string[] // IDs de grupos a los que pertenece (puede estar en m√∫ltiples grupos)
  createdAt?: any
  updatedAt?: any
}
...
export function useAdminUsers(user: any) {
  ...
  const actualizarLocationId = useCallback(async (userId: string, locationId: string) => {
    ...
    try {
      ...
      toast({
        title: "UbicaciÛn actualizada",
        description: "La ubicaciÛn del usuario fue actualizada",
      })
      ...
      toast({
        title: "Error",
        description: error.message || "No se pudo actualizar la ubicaciÛn",
        variant: "destructive",
      })
      return false
    }
